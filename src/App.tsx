...
function App() {
  ...
  return <div className="app-shell">
    <aside className="sidebar"><div className="brand"><span className="brand-mark">K</span><span>Kath<span className="brand-accent">Ledger</span></span></div><div className="workspace-label">WORKSPACE</div><nav>{navItems.slice(0, 5).map((item) => <NavButton key={item.view} item={item} activeView={activeView} setActiveView={setActiveView} />)}</nav><div className="workspace-label settings-label">MANAGE</div><nav>{navItems.slice(5).map((item) => <NavButton key={item.view} item={item} activeView={activeView} setActiveView={setActiveView} />)}</nav><div className="sidebar-footer"><div className="sync-dot" /><div><strong>{loading ? 'Syncing...' : 'Google Sheets'}</strong><small>{loading ? 'Loading records' : 'Connected workspace'}</small></div></div></aside>
    <main className="main-content"><header className="topbar"><div><span className="eyebrow">SEPTEMBER 2026</span><h1>{activeView === 'Overview' ? 'Good Day, KathLeng.' : activeView}</h1></div><div className="header-actions"><button className="icon-button" aria-label="Notifications">♧</button><button className="avatar">AK</button></div></header>
