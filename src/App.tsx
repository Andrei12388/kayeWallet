...
function App() {
  ...
  const overviewWeeklyData = Array.from({ length: 7 }, (_, index) => {
    const date = new Date(`${selectedWeek.start}T00:00:00`)
    date.setDate(date.getDate() + index)
    const dateValue = dateKey(date)
    const dayTransactions = overviewTransactions.filter((transaction) => transaction.date === dateValue)
    return {
      label: new Intl.DateTimeFormat('en-US', { weekday: 'short' }).format(date),
      income: totalByType(dayTransactions, 'INCOME'),
      expenses: totalByType(dayTransactions, 'EXPENSE'),
    }
  })
    ...
    <main className="main-content"><header className="topbar"><div><span className="eyebrow">{activeView === 'Overview' ? monthName(selectedMonth).toUpperCase() : 'SEPTEMBER 2026'}</span><h1>{activeView === 'Overview' ? 'Good Day, KathLeng.' : activeView}</h1></div><div className="header-actions"><button className="icon-button" aria-label="Notifications">♧</button><button className="avatar">KW</button></div></header>
      {activeView === 'Categories' ? <CategoriesView categories={categories} onChanged={async (message) => { await reloadCategories(); notify('success', message) }} onError={(message) => notify('error', message)} /> : activeView === 'Allocation' ? <AllocationView allocations={allocations} income={income} onSaved={(next) => { setAllocations(next); notify('success', 'Allocations saved.') }} onError={(message) => notify('error', message)} /> : activeView === 'Reports' ? <MonthlySummary transactions={transactions} years={reportYears} year={reportYear} setYear={setReportYear} /> : <>
        ...
        <section className="summary-grid">
          ...
          <SummaryCard label="Total expenses" amount={overviewExpenses} tone="expense" detail="Selected week" /></section><section className="content-grid"><ReportChart className="chart-panel" title="Weekly transactions" subtitle={selectedWeek.label} data={overviewWeeklyData} emptyMessage="No transactions recorded in this week." /><div className="panel allocation-panel">
...
function MonthlySummary({ transactions, years, year, setYear }: { transactions: Transaction[]; years: number[]; year: number; setYear: (year: number) => void }) {
  ...
  const weekly = Array.from(transactions.reduce((totals, transaction) => {
    const date = new Date(`${transaction.date}T00:00:00`)
    if (date.getFullYear() !== year || date.getMonth() !== weeklyMonth) return totals
    const start = new Date(date)
    const day = start.getDay()
    start.setDate(start.getDate() - (day === 0 ? 6 : day - 1))
    const key = dateKey(start)
    const current = totals.get(key) || { label: `${monthLabel(start.getMonth())} ${start.getDate()}`, income: 0, expenses: 0 }
    if (transaction.type === 'INCOME') current.income += transaction.amount
    else current.expenses += transaction.amount
    totals.set(key, current)
    return totals
  }, new Map<string, { label: string; income: number; expenses: number }>()).entries()).sort((first, second) => first[0].localeCompare(second[0])).map(([, item]) => item)
  ...
  return <div className="report-page"><div className="page-toolbar"><div><p className="page-kicker">MONTHLY SUMMARY</p><p className="muted">Compare income, expenses, and balance by month.</p></div><label className="year-picker">Year<select value={year} onChange={(event) => setYear(Number(event.target.value))}>{years.map((option) => <option key={option} value={option}>{option}</option>)}</select></label></div><section className="summary-grid"><SummaryCard label="Year income" amount={yearIncome} tone="income" detail={`${year} total`} /><SummaryCard label="Year expenses" amount={yearExpenses} tone="expense" detail={`${year} total`} /><SummaryCard label="Year balance" amount={yearIncome - yearExpenses} tone="balance" detail={`${year} net`} /></section><section className="report-charts"><ReportChart title="Weekly transactions" subtitle="Total income and expenses by week" data={weekly} month={weeklyMonth} onMonthChange={setWeeklyMonth} emptyMessage={`No weekly activity recorded in ${monthLabel(weeklyMonth)} ${year}.`} /><ReportChart title="Monthly transactions" subtitle="Total income and expenses by month" data={monthly.map((item, month) => ({ label: monthLabel(month), ...item }))} /></section><section className="panel monthly-summary"><div className="panel-heading"><div><h2>{year} monthly activity</h2><p className="muted">All recorded transactions for the selected year.</p></div></div><div className="monthly-table-wrap"><table><thead><tr><th>Month</th><th className="amount-column">Income</th><th className="amount-column">Expenses</th><th className="amount-column">Balance</th></tr></thead><tbody>{monthly.map((item, month) => <tr key={month}><td><strong>{monthLabel(month)}</strong></td><td className="amount income">+{formatCurrency(item.income)}</td><td className="amount expense">-{formatCurrency(item.expenses)}</td><td className={`amount ${item.income - item.expenses >= 0 ? 'income' : 'expense'}`}>{formatCurrency(item.income - item.expenses)}</td></tr>)}</tbody></table></div></section><section className="panel monthly-summary"><div className="panel-heading"><div><h2>Category totals</h2><p className="muted">Income and expenses combined by category for {year}.</p></div></div><div className="monthly-table-wrap"><table><thead><tr><th>Category</th><th className="amount-column">Income</th><th className="amount-column">Expenses</th><th className="amount-column">Net</th></tr></thead><tbody>{categoryTotals.map((item) => <tr key={item.name}><td><strong>{item.name}</strong></td><td className="amount income">+{formatCurrency(item.income)}</td><td className="amount expense">-{formatCurrency(item.expenses)}</td><td className={`amount ${item.income - item.expenses >= 0 ? 'income' : 'expense'}`}>{formatCurrency(item.income - item.expenses)}</td></tr>)}{categoryTotals.length === 0 && <tr><td colSpan={4} className="empty-state">No category activity for this year.</td></tr>}</tbody></table></div></section></div>
}

function ReportChart({ className, title, subtitle, data, month, onMonthChange, emptyMessage }: { className?: string; title: string; subtitle: string; data: { label: string; income: number; expenses: number }[]; month?: number; onMonthChange?: (month: number) => void; emptyMessage?: string }) {
  const maximum = Math.max(...data.flatMap((item) => [item.income, item.expenses]), 1)
  return <section className={`panel report-chart ${className || ''}`}><div className="panel-heading"><div><h2>{title}</h2><p className="muted">{subtitle}</p></div><div className="report-chart-actions">{onMonthChange && month !== undefined && <label className="chart-period">Month<select value={month} onChange={(event) => onMonthChange(Number(event.target.value))}>{Array.from({ length: 12 }, (_, option) => <option key={option} value={option}>{monthLabel(option)}</option>)}</select></label>}<div className="legend"><span><i className="legend-income" />Income</span><span><i className="legend-expense" />Expense</span></div></div></div>{data.length === 0 ? <div className="chart-empty">{emptyMessage}</div> : <div className="report-bars">{data.map((item) => <div className="report-bar-group" key={item.label}><div className="report-bar-pair"><div className="report-bar-column"><span className="report-bar-value">{formatCurrency(item.income)}</span><div className="report-chart-bar income-bar" style={{ height: `${item.income / maximum * 100}%` }} title={`Income ${formatCurrency(item.income)}`} /></div><div className="report-bar-column"><span className="report-bar-value">{formatCurrency(item.expenses)}</span><div className="report-chart-bar expense-bar" style={{ height: `${item.expenses / maximum * 100}%` }} title={`Expense ${formatCurrency(item.expenses)}`} /></div></div><small>{item.label}</small></div>)}</div>}</section>
}
