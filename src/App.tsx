...
function App() {
  ...
  return <div className="app-shell">
    <aside className="sidebar"><div className="brand"><span className="brand-mark">K</span><span>Kaye<span className="brand-accent">Wallet</span></span></div><div className="workspace-label">WORKSPACE</div><nav>{navItems.slice(0, 5).map((item) => <NavButton key={item.view} item={item} activeView={activeView} setActiveView={setActiveView} />)}</nav><div className="workspace-label settings-label">MANAGE</div><nav>{navItems.slice(5).map((item) => <NavButton key={item.view} item={item} activeView={activeView} setActiveView={setActiveView} />)}</nav><div className="sidebar-footer"><div className="sync-dot" /><div><strong>{loading ? 'Syncing...' : 'Google Sheets'}</strong>
